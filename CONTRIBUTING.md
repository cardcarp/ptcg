...
3. Run the compile script to verify your YAML syntax and schema compliance: `cardcarp-compile`
...
### `regulation` is authored on the card, pooled on the oracle

A card's regulation mark belongs to its printing, not its mechanics — `Energy Switch` has been printed under `D`, `F`, `G` and `I`. So the mark is authored on the card, as a single value, and is null for any printing older than the mechanic.

`compile.py` gathers every mark an oracle has appeared under from its printings and attaches that pool to each joined card record as `oracle_regulation`. Legality tests the pool, because a player may use any printing whose mark is legal in the current format. Storing the pool on the oracle instead was the earlier design, and it back-propagated modern marks onto vintage printings — a 2007 `Dusk Ball` claimed the `H` from its 2024 reprint.
