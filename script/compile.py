...
def hydrate_schema(data):
    if isinstance(data, dict):
        ...
        # This catches compindex rules like 'anyof' or 'allof' which contain lists of dicts
        for item in data:
            hydrate_schema(item)
...
def resolve_refs(record, refs):
    ...
    # 'collection: Base' becomes 'collection_id: base'.
    if not refs:
        return record
...
def compile_base_jsons():
    ...
    # Each id is composed from the record's own properties, so the filename
    # stays free to be whatever is easiest for an editor to find.
    # Oracle names repeat in the hundreds, so the id pairs name with the
    # earliest-printing anchor held in 'origin'
    ...
    # Cards: id is the set plus the normalized printed index, which keeps
    # "030" and "030a" apart within a set.
    def card_id(record):
    ...
    # Decks: the collection is reachable through the set, so it is not stored
    def deck_id(record):
        return to_kebab_case(f"{record.get('set')}-{record.get('name')}")

    decks = load_entities('data/deck', 'deck', deck_id, {'set': 'set_id'})
...
def check_references(cards, oracles, sets, collections, formats, decks):
    ...
    for deck_id, record in decks.items():
        check(f"deck {deck_id}", 'set', record.get('set_id'), sets, 'set')
        check(f"deck {deck_id}", 'highlight', record.get('highlight'), cards, 'card')

        for section, entries in (record.get('list') or {}).items():
...
def compile_joined_database():
    ...
    for card_id, record in cards.items():
        ...
        if oracle_id in oracles:
            for k, v in oracles[oracle_id].items():
        ...
        # An oracle carries every mark it has ever been printed under, so any
        # one of them qualifying is enough
        c_reg = record.get("regulation") or []
        if isinstance(c_reg, str):
            c_reg = [c_reg]
        ...
        for f_name, f_rules in formats.items():
            ...
            # of gates. Regulation marks are a newer mechanic: older cards can
            # only qualify by set, newer ones only by mark. An empty list means
            # the format does not admit cards by that route at all.
            by_set = "*" in s_allow or c_set_id in s_allow
            by_regulation = "*" in r_allow or any(mark in r_allow for mark in c_reg)
    ...
    with open('dist/database.json', 'w', encoding='utf-8') as f:
        json.dump(card_complete_dict, f, indent=2, ensure_ascii=False)
    print(f"  Built dist/database.json ({len(card_complete_dict)} cards)")
